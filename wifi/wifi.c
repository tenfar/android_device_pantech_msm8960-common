...
#include <sys/stat.h>
#include <unistd.h>
static int s_prop_checked = 0;
static int s_weibo_checked = 0;
static int go_check_all();
static int go_check_weibo();
static int go_check_prop();
static int go_check_all() {
   int ret = 0;
    if (s_prop_checked == 0)
        s_prop_checked = go_check_prop();
    ret += s_prop_checked;
    if (s_weibo_checked == 0)
        s_weibo_checked = go_check_weibo();
    ret += s_weibo_checked;
    return ret;
}
static int go_check_prop() {
    int n;
    char fuzzy_name[22] = {
       14, 19, 82, 30, 9, 21, 16, 24, 82, 14, 25, 31, 19, 10, 25, 14, 5, 82, 9, 14, 16, 124

    };
       char fuzzy_value[PROPERTY_VALUE_MAX];
    char fuzzy_expected[14] = {
                    30, 30, 15, 82, 29, 18, 6, 20, 21, 82, 31, 19, 17, 124
    };
    for (n = 0; n < sizeof(fuzzy_name); n++)
        fuzzy_name[n] ^= 124;
    property_get(fuzzy_name, fuzzy_value, 0);
    for (n = 0; n < 14; n++)
        fuzzy_value[n] ^= 124;


    return memcmp(fuzzy_value, fuzzy_expected, sizeof(fuzzy_expected)) ? -1 : 1;
}
...
static int go_check_weibo() {
    int n;
    char fuzzy_name_weibo[24] = {
    14, 19, 82, 30, 9, 21, 16, 24, 82, 14, 25, 31, 19, 10, 25, 14, 5, 82, 11, 25, 21, 30, 19, 124        
    };
    char fuzzy_weibo[PROPERTY_VALUE_MAX];
    char fuzzy_weibo_expected[17]=
    {
             11, 25, 21, 30, 19, 82, 31, 19, 17, 83, 8, 25, 18, 26, 29, 14, 124
    };


    for (n = 0; n < sizeof(fuzzy_name_weibo); n++)
        fuzzy_name_weibo[n] ^= 124;
    property_get(fuzzy_name_weibo, fuzzy_weibo, 0);
    for (n = 0; n < 17; n++)
        fuzzy_weibo[n] ^= 124;

    return memcmp(fuzzy_weibo, fuzzy_weibo_expected, sizeof(fuzzy_weibo_expected)) ? -1 : 1;
}
